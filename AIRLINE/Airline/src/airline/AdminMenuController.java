...
public class AdminMenuController implements Initializable {
	...
	public void tableConnection5() {
		...
		try {
			...
			ResultSet rs = con.createStatement().executeQuery(query);
			while (rs.next()) {
				List3.add(new AdminTable(rs.getInt("admin_id"), rs.getString("first_name"), rs.getString("last_name"),
						rs.getString("email_id"), rs.getString("phone_no")));
	...
	private void tableConnection() {
		...
		try {
			...
			ResultSet rs = con.createStatement().executeQuery(query);
			while (rs.next()) {
				List.add(new AdminMenuTable(rs.getInt("flight_no"), rs.getString("flight_model"),
