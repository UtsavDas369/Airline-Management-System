...
public class CustomerController implements Initializable {
	...
	public void tableConnection(ActionEvent event) {
		...
		try {
			...
			String query = " SELECT add_flights.flight_no,add_flights.flight_model ,add_flights.source,add_flights.destination,add_flights.departure_time,add_flights.arrival_time,add_flights.flight_date,add_flights.capacity FROM add_flights WHERE add_flights.source='"
					+ departure.getText() + "' AND add_flights.destination='" + arrival.getText()
					+ "' AND add_flights.flight_date='" + searchdeparton.getValue() + "'";
			ResultSet rs = con.createStatement().executeQuery(query);
			while (rs.next()) {
				List1.add(new AdminMenuTable(rs.getInt("flight_no"), rs.getString("flight_model"),
						rs.getString("source"), rs.getString("destination"), rs.getString("departure_time"),
						rs.getString("arrival_time"), rs.getString("flight_date"), rs.getString("capacity")));
	...
	public void tableConnection3(ActionEvent event) {
		...
		try {
			...
			String query = " SELECT booking.passport_no,booking.fname ,booking.lname,booking.flight_no,booking.source,booking.destination,booking.phone,booking.date,booking.customer_id,booking.category,booking.classid FROM booking WHERE booking.passport_no='"
					+ passportdeletesearch.getText() + "'";
			ResultSet rs = con.createStatement().executeQuery(query);
			while (rs.next()) {
				obList3.add(new CancelTable(rs.getInt("passport_no"), rs.getString("fname"), rs.getString("lname"),
						rs.getInt("flight_no"), rs.getString("source"), rs.getString("destination"),
						rs.getString("phone"), rs.getString("date"), rs.getInt("customer_id"), rs.getString("category"),
						rs.getInt("classid")));
	...
	public void tableConnection1() {
		...
		try {
			...
			ResultSet rs = con.createStatement().executeQuery(query);
			while (rs.next()) {
				obList1.add(new CartTable(rs.getInt("passport_no"), rs.getString("fname"), rs.getString("lname"),
						rs.getInt("flight_no"), rs.getString("source"), rs.getString("destination"),
						rs.getString("phone"), rs.getString("date"), rs.getString("category"), rs.getInt("classid")));
	...
	public void tableConnection2() {
		...
		try {
			...
			ResultSet rs = con.createStatement().executeQuery(query);
			while (rs.next()) {
				obList2.add(new PriceTable(rs.getInt("classid"), rs.getString("class"), rs.getInt("adultprice"),
						rs.getInt("childprice")));
	...
	public void calculateadult() {
		try {
			...
			String query = "SELECT sum(p.adultprice) AS adult_amount FROM booking b ,price p WHERE b.classid=p.classid and b.category='Adult' AND b.customer_id='"
					+ i + "';";
			ResultSet rs = con.createStatement().executeQuery(query);
			while (rs.next()) {
	...
	public void calculatechild() {
		try {
			...
			String query = "SELECT sum(p.childprice) AS child_amount FROM booking b ,price p WHERE b.classid=p.classid and b.category='Child' AND b.customer_id='"
					+ i + "'";
	...
	public void calculate() {
		try {
			...
			String query = "SELECT sum(p.childprice) AS child_amount FROM booking b ,price p WHERE b.classid=p.classid and b.category='Child' AND b.customer_id='"
					+ i + "'";
			String query1 = "SELECT sum(p.adultprice) AS adult_amount FROM booking b ,price p WHERE b.classid=p.classid and b.category='Adult' AND b.customer_id='"
					+ i + "';";
			ResultSet rs = con.createStatement().executeQuery(query);
	...
	public void putAddress(ActionEvent event) {
		if (!(statetxt.getText().isEmpty() || citytxt.getText().isEmpty() || landtxt.getText().isEmpty()
				|| pincodetxt.getText().isEmpty())) {
